...
sub makePlayerCode
{
    ...
    if ($ENV{REQUEST_URI} =~ /[\?\&]t=(\d+)/) {
        $extra_onplay .= qq|player.api('seek',$1);|;
        $file->{autostart} = 1;
    }
...
function PlayerjsEvents(event, id, data) {
    console.log("PlayerJS Event:", event, id, data); // Debug: Verify event triggering
    if (event == "play") {
        console.log("Play event detected, calling doPlay()");
        return doPlay();
    }
    if (event == "end") {
        return doEnd();
...
function doPlay(pl) {
    console.log("doPlay() started, vvplay:", vvplay); // Debug: Check if function runs
    \$('div.video_ad').hide();
    if (vvplay) {
        console.log("doPlay() skipped: already played");
        return;
    }
    vvplay = 1;
    var adb = 0;
    if (window.adbon == 1) { adb = 1; }
    console.log("Sending AJAX to count view: $c->{site_url}/dl?op=view&file_code=$file->{file_code}&hash=$file->{ophash}&embed=$f->{embed}&adb=" + adb);
    \$.get('$c->{site_url}/dl?op=view&file_code=$file->{file_code}&hash=$file->{ophash}&embed=$f->{embed}&adb=' + adb, function(data) {
        console.log("AJAX response received:", data); // Debug: Log server response
        \$('#fviews').html(data);
    }).fail(function(jqXHR, textStatus, errorThrown) {
        console.error("AJAX request failed:", textStatus, errorThrown); // Debug: Log errors
    });
    $extra_onplay
}

function doTime(currentTime) {
    if ($time_fadein > 0 && currentTime >= $time_fadein && vvad != 1) {
        vvad = 1;
...
;
    ...
    # Time Slider (Thumbnails) Implementation
    if ($c->{m_z} && $c->{time_slider}) {
        # Calculate interval like VideoJS: total duration / (columns * rows)
        my $interval = sprintf("%.1f", $file->{vid_length} / ($c->{m_z_cols} * $c->{m_z_rows}));
        my $sprite_url = "$file->{video_img_folder}/$file->{file_real}0000.jpg";
        $extra_settings .= qq[, thumbnails: {
            url: "$sprite_url",
            width: $c->{thumb_width},
