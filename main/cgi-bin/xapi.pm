...
sub run
{
	...
	my $sub={
		...
		adm_delete_file	=> \&AdminDeleteFile,
...
sub UploadURL
{
	...
	my $code;
	do {$code = randchar(12)} while ($db->SelectOne("SELECT file_id FROM Files WHERE file_code=? OR file_real=?",$code,$code));
...
sub FolderList
{
  ...
  for(@$fld_list)
  {
    push @folders, {fld_id=>$_->{fld_id}, name=>$_->{fld_name}, fld_code=>$_->{fld_code}};
  }
...
sub canPlay
{
    ...
    return 1 if $file->{file_size_o} && $file->{file_spec_o}=~/(mp4)$/i && $file->{file_spec_o}=~/h264/i && $file->{file_spec_o}=~/(aac|mp3)/i;
    return 0;
}
...
sub FileEdit
{
    ...
    for(@$files)
    {
    	...
    	$ses->addTagsToFile( $f->{tags}, $_->{file_id} );
    }
...
sub FileDirectLink
{
	...
	if($f->{q}=~/^\w$/)
	{
		$s = $file->{"file_size_$f->{q}"} ? $ses->genDirectLink( $file, $f->{q}, "$file->{file_title}.mp4" ) : "";
	}
	else
	{
		my @arr;
...
sub FolderCreate
{
   ...
   my $code;
   do {$code = $ses->randchar(10)} while ($db->SelectOne("SELECT fld_id FROM Folders WHERE fld_code=?",$code));
   ...
   return out({"status"=>200, "msg"=>"OK", result=>{fld_id => $fld_id, fld_code=>$code} });
}
...
sub UploadSub
{
	...
	elsif($f->{sub_url} && $c->{srt_allow_url_upload})
	{
	...
	my $no_db_update = $file->{file_captions} && $f->{sub_lang}=~m/$file->{file_captions}/ ? 1 : 0;
