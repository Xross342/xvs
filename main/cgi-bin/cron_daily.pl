...
{
   $db->Exec("DELETE FROM Views WHERE created<NOW() - INTERVAL ? DAY",$c->{clean_ip2files_days});
...
$db->Exec("DELETE FROM StatsCountry WHERE day<CURDATE() - INTERVAL 14 DAY");

$db->Exec("DELETE FROM FilesTrash WHERE file_deleted<NOW() - INTERVAL 30 DAY");
