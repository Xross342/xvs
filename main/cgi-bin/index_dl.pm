...
sub Download1
{
	...
	$file->{user_avatar_url} = "$c->{site_url}/upload-data/avatar_$file->{usr_id}.jpg" if -e "$c->{site_path}/upload-data/avatar_$file->{usr_id}.jpg";
...
sub VideoMakeCode
{
	...
	{
		$file->{msg2} = "$ses->{lang}->{lng_download_video_processing_now} <span id='enc_pp'>...</span>";
		$file->{video_wait}=1;
		$file->{enc_status}=1;
		return $file;
	}
	...
	my @dlinks;
	my @qletters = $preview ? ('p') : (@{$c->{quality_letters}},'o');
	for my $q (@qletters)
	{
...
sub Embed
{
   ...
   my $tmpl = $ses->isMobile ? "video_embed_mobile.html" : "video_embed.html";
...
sub CommentsList
{
   ...
   my $list = $db->SelectARefCached(15,"SELECT *, INET_NTOA(cmt_ip) as ip, DATE_FORMAT(created,'%M %e, %Y at %r') as date
                               FROM Comments 
                               WHERE cmt_type=? 
                               AND cmt_ext_id=?
