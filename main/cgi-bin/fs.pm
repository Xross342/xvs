...
if($f->{sid})
{
   ...
   if($tt)
   {
	   ...
	   #$db->Exec("UPDATE Torrents SET status='DONE' WHERE sid=?",$tt->{sid});
   }
}
...
$user ||= $db->SelectRow("SELECT u.*, UNIX_TIMESTAMP(usr_premium_expire)-UNIX_TIMESTAMP() as exp_sec
                        ...
                        WHERE s.stream_code=?
                        AND s.usr_id=u.usr_id",$f->{name}) if !$user && $f->{app} && $f->{name}; # Stream
...
sub CheckCodes
{
	...
	#my $ok = $db->SelectARef("SELECT file_real FROM Files WHERE file_real IN (".join(',', map{"'$_'"}@codes2 ).")");
	my $ok=[];
	if($f->{host_id}=~/^\d+$/)
	{
...
sub TorrentAdd
{
   ...
   #return sendBack("ERROR:This type of users is not allowed to upload torrents") unless $c->{torrent_dl_slots};
...
{
	#require Data::Dumper;
	#logg( "TorrentStats: ".Data::Dumper->Dump([$f]) );
...
{
	require Data::Dumper;
	#logg( "TorrentDone: ".Data::Dumper->Dump([$f]) );
...
sub SaveFile
{
   ...
   unless($f->{no_limits})
   {
		...
		#$c->{disk_space}=0 if $user && $user->{usr_adm};
		if($disk_space_max)
		{
   ...
   # {
   #    return saveFileError("this file is banned by administrator");
   # }
   ...
   $db->Exec("INSERT INTO Files 
	...
	$size=0 unless $code eq $real; # is copy here
	...
	if($c->{m_g} && $c->{srt_on} && $c->{srt_auto} && $f->{data_srt})
	{
		...
		for(split(/\^\^\^/,$f->{data_srt}))
		{
			...
			# my $dx = sprintf("%05d",($real_id||$file_id)/$c->{files_per_folder});
			...
			push @langsaved, $lng; #if !$lngdone->{$lng}++;
		}
		#$db->Exec("UPDATE Files SET file_captions=? WHERE file_id=?", join('|',@langsaved), $file_id );
	}

	my $edata; # extra data for XUpload.pm
	if($code eq $real)
	{
		my $no_encoding;
	...
	#$no_encoding=1 if $server->{srv_type} eq 'UPLOADER'  && !$server->{srv_encode};

	$no_encoding=1 if $c->{video_extensions} && $filename!~/\.($c->{video_extensions})$/i;
	...
	$no_encoding=1 if $c->{allow_no_encoding} && 
     ...
     if($no_encoding) # mp4/flv file already
     {
     ...
     else
     {
         ...
         {
             ### Seems this is STORAGE with no Encoding
             ### Just leave the original there then
         }
     }
...
sub findServerEncoder
{
   ...
   # try to find STORAGE with encoding
   if($#$srv_ids2==-1)
   {
   ...
   # Choose server with minimal encode/transfer jobs
...
sub AddEncodeQueue
{
	...
	require XUtils;
	my $vi = XUtils::vInfo($file,'o'); # vid_bitrate vid_audio_bitrate vid_codec vid_audio_codec

	my $audio_codec_pass=1 if $vi->{vid_audio_codec} eq 'AAC';
	my $video_codec_pass=1 if $vi->{vid_codec} eq 'H264';
	my $container_pass=1 if lc($vi->{vid_container}) eq 'mp4';

	#return if $c->{allow_no_encoding} && $file->{usr_id} && $video_codec_pass && $audio_codec_pass && $container_pass && $db->SelectOne("SELECT value FROM UserData WHERE usr_id=? AND name='usr_no_encoding'",$file->{usr_id});
    ...
    else
    {
		...
		for my $i (0..$#qlist)
		{
			...
			$pass1=1 if $i==$#qlist; # always encode lowest quality
...
sub QueueEncNext
{
   ...
   #$settings->{multi_audio_on}=$settings->{default_audio_lang}='' if !$c->{m_8} && ($settings->{multi_audio_on}||$settings->{default_audio_lang});
   delete @{$settings}{'multi_audio_on','default_audio_lang'} unless $c->{m_8};

   if($c->{m_v}) # watermark mod
   {
       ...
       $settings->{usr_id} = $usr_id_dat; #$user->{usr_id};

       my $pass=1 if $c->{m_v_users} eq 'admin'; # && $user->{usr_adm};
       ...
       if($pass)
       {
           ...
           #my $sett = join '|', map{"$_=$dat->{$_}"} grep {/^watermark_/} keys %$dat;
           #$quality.="|$sett" if $sett;
       }
   }
...
sub QueueEncDone
{
   ...
   # last quality for this video encoded
   if(!$other_qualities)
   {
      my $keep_orig = $c->{vid_keep_orig};

      # don't keep original if encoded version same or higher resolution
      require XUtils;
      my $info_orig = XUtils::vInfo($file,'o');
      my $bestq;
      for(@{$c->{quality_letters}}){ $bestq||=$_ if $file->{"file_size_$_"}; }
      my $info_best = XUtils::vInfo($file,$bestq);
      $keep_orig=0 if $info_orig->{vid_height} <= $info_best->{vid_height}; # Don't keep Orig when encoded dimensions are same or higher

      $keep_orig=0 if $c->{vid_keep_orig_playable} && !($file->{file_spec_o}=~/mp4$/i && $file->{file_spec_o}=~/h264/i && $file->{file_spec_o}=~/aac/i);

	# Delete original if all Q encodings are finished and we don't keep Orig
	unless($keep_orig)
	{
      ...
      # Now let's move file to STORAGE if required
...
# sub genDirectLinkTransfer
# {
# 	my ( $file, $quality, $fname )=@_;
...
# 	#my ($ip1,$ip) = ('0.0','0.0');
# 	my ($ip1,$ip) = $ses->genIPLogic( $ses->getIP );
...
sub QueueTransferNext
{
	...
	my $list = $db->SelectARef("SELECT srv_id FROM Servers 
								...
								AND srv_status<>'OFF'
								AND srv_disk<srv_disk_max*0.95");
	my $servers = join ',', map{$_->{srv_id}} @$list; # target servers
	return unless $servers;
	...
	{
		# Avoid 2 parallel transfers to same HDD
		my $active = $db->SelectARef("SELECT * FROM QueueTransfer
		                             WHERE status='MOVING'
		                             AND UNIX_TIMESTAMP()-UNIX_TIMESTAMP(updated) < 60");
		$active_list = join ',', map{$_->{srv_id2}} @$active; # current active targets
		$active_list2 = join ',', map{$_->{file_real_id}} @$active;
	}
	$active_list||=0;
	$active_list2||=0;

	# my $srv_id1 = $db->SelectOne("SELECT srv_id1 
	# 								FROM QueueTransfer 
	...
	$ses->{transfer_speed} = $srv2->{host_transfer_speed} || $c->{server_transfer_speed} || 25000; # KB, 25000 = 250 mbit/s

	my @links;
	$ses->{no_ip_check}=1;
	for my $q (@{$c->{quality_letters}},'o','p')
	{
		#push @links, 'ENC|'.genDirectLinkTransfer($file, $q, "$file->{file_real}_$q").'|'.$file->{"file_size_$q"} if $file->{"file_size_$q"};
...
sub QueueTransferDone
{
   ...
   $db->Exec("UPDATE QueueEncoding SET srv_id=? WHERE file_real=?",$qq->{srv_id2},$qq->{file_real}); # just in case it was queued to encode before
   ...
   #if(!$file->{file_size_n} && !$file->{file_size_l} && !$file->{file_size_h})
   ...
   #my $res = $ses->api2( $qq->{srv_id1}, { op=>'del_files', list=>"$qq->{file_real_id}-$qq->{file_real}" } );
...
sub QueueURLNext
{
	...
	# Skip users with X+ working uploads
	...
	if($c->{optimize_hdd_perfomance})
	{
		# Avoid 2 parallel url uploads to same HDD
		my $active = $db->master()->SelectARef("SELECT srv_id FROM QueueUpload
										WHERE status='WORKING'
										AND UNIX_TIMESTAMP()-UNIX_TIMESTAMP(updated) < 60");
		$active_list = join ',', map{$_->{srv_id}} @$active; # current active targets
	}
	$active_list||=0;
	my $IP = $ses->getIP;
...
# {
#    my ($file,$mode,$fname)=@_;
#    my $speed = $file->{host_transfer_speed} || $c->{server_transfer_speed} || 15000; # KB, 15000 = 150 mbit/s
#    require HCE_MD5;
#    my $hce = HCE_MD5->new($c->{dl_key},"XVideoSharing");
#    my $usr_id = 0;
#    my $dx = sprintf("%d",($file->{file_real_id}||$file->{file_id})/$c->{files_per_folder});
...
sub UpdateFileData
{
    ...
    # {
    #     next if $f->{$_}=~/\'/;
    #     push @data, "$_='$f->{$_}'" if defined $f->{$_};
    # }
    #$db->Exec("UPDATE Files SET ".join(',',@data)." WHERE file_real=?",$f->{file_code}) if @data;
...
sub DeleteDiskNext
{
	#my $srv_ids = $db->SelectOneCached("SELECT GROUP_CONCAT(srv_id) FROM Servers WHERE host_id=?",$f->{host_id});
