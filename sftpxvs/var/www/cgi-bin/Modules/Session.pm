...
#$CGI::Simple::POST_MAX = 10*1024*1024;
use XFileConfig;
use DataBase;
use JSON::XS;
#use Net::SSL; # may be required for HTTPS, also install Mozilla::CA, LWP::Protocol::https
...
sub initCGI
{
	...
	my $in = $query ? $query : new CGI(); #::Simple();
	$self->{cgi_query} = $in;
	$self->{form}={};
	$self->{cookies}={};
	#$in->autoEscape(0);
	...
	for( $in->param() )
	{
		...
		#my @val = $in->can('multi_param') ? $in->multi_param($_) : $in->param($_);
	...
	$self->{cookies}->{$_}=$in->cookie($_) for $in->cookie();
	$self->{domain}=$c->{domain} || $ENV{HTTP_HOST};
	$self->{domain}=~s/^(www|main|srt)\.//;
	$self->{domain}=~s/:.*$//;
	$c->{cdn_url}||=$c->{site_url};

	#$ENV{HTTP_USER_AGENT} = $self->SecureStr($ENV{HTTP_USER_AGENT});
	...
	$self->{language} = $self->{cookies}->{lang} if $self->{cookies}->{lang}=~/^\w+$/;
	$self->{language}||=$c->{default_language};
	$self->{language}='English' unless $self->{language}=~/^([a-z]{3,20})$/i;
	$self->{language}='English' unless -f "$c->{cgi_path}/Templates/static/X$self->{language}.pm";

	if(-f "$c->{cgi_path}/Templates/static/X$self->{language}.pm")
	{
		require "$c->{cgi_path}/Templates/static/X$self->{language}.pm";
		my $package="X$self->{language}";
		my $ttt = $package->ttt;
		$self->{lang}->{$_} = $ttt->{$_} for keys %$ttt;
	}

	# if(!$c->{no_lng_sql} && $c->{db_login})
	# {
	# 	my $list = $self->db->SelectARefCached(120,"SELECT * FROM Translations WHERE lang_id=1");
...
{
	my ($self) = @_;
	my $selfs_id = $self->getCookie( $self->{auth_cook} ) || $self->f->{sess_id};
	my $f = $self->f;
	my $db = $self->db;
	return undef unless $selfs_id;

	my $extra_fields="DATE_FORMAT(usr_premium_expire,'%e %M %Y') as premium_expire, usr_security_lock, usr_money, usr_pay_email, usr_pay_type, usr_api_key, usr_channel_name, usr_no_file_delete, usr_password_changed," if $self->f->{op}=~/^(my_account|my_password|my_email|request_money|payments|my_reseller|api_reseller|api_reference)$/;
		$extra_fields.="usr_no_file_delete, usr_disk_space, usr_disk_used, usr_files_used," if $self->f->{op}=~/^(my_files|dashboard)$/;

	$self->{user} = $db->SelectARefCachedKey("ses$selfs_id", 180, 
											"SELECT s.session_id, u.usr_id, usr_login, usr_email, usr_adm, usr_mod, usr_status, usr_reseller, usr_notes, usr_premium_only, usr_allowed_ips, usr_uploads_on, usr_vip,
												$extra_fields
												UNIX_TIMESTAMP(usr_premium_expire)-UNIX_TIMESTAMP() as exp_sec,
												UNIX_TIMESTAMP()-UNIX_TIMESTAMP(last_time) as dtt
											FROM Users u, Sessions s 
											WHERE s.session_id=? 
											AND s.usr_id=u.usr_id",$selfs_id)->[0];
	unless($self->{user})
	{
		return undef;
	}
	if($self->{user}->{usr_status} eq 'BANNED')
	{
		delete $self->{user};
		$self->{returning}=1;
		return $self->message($self->{lang}->{lng_login_your_account_banned});
	}
	if($self->{user}->{dtt}>180)
	{
		$db->Exec("UPDATE Sessions SET last_time=NOW() WHERE session_id=?",$selfs_id);
		$db->Exec("UPDATE Users SET usr_lastlogin=NOW(), usr_lastip=INET6_ATON(?) WHERE usr_id=?", $self->getIP, $self->{user}->{usr_id} );
	}
	$self->{user}->{premium}=1 if $self->{user}->{exp_sec}>0;
	if($c->{email_validation_code} && $self->{user}->{usr_login_code} && $self->f->{op}!~/^(login_code|logout)$/)
	{
		$self->{returning}=1;
		return $self->PrintTemplate("login_code.html", %{$self->{user}});
	}
	if($c->{m_d} && $self->{user}->{usr_mod})
	{
		$self->{lang}->{$_}=$c->{$_} for qw[m_d_f m_d_a m_d_c m_d_featured m_d_file_approve];
		$self->{lang}->{usr_mod}=1;
	}
	if($c->{m_d} && $self->{user}->{usr_notes}=~/LEGAL=\d+/i)
	{
		$self->{lang}->{usr_legal}=1;
	}
	#$self->setCookie( $self->{auth_cook} , $selfs_id );
	return $self->{user};
}
...
sub CreateTemplate
{
    ...
    my $t=HTML::Template->new( filename => "$c->{cgi_path}/Templates$design/$filename",
                                die_on_bad_params => 0,
                                global_vars => 1,
                                loop_context_vars => 1,
                                utf8     => 1,
    ...
    if( $c->{m_5} && !$self->{adm} )
    {
        ...
        $hh->{m_5_devtools_redirect} = $c->{m_5_devtools_redirect};
    ...
    $hh->{hide_menu}=1 if $self->{form}->{op}=~/^(login|forgot_pass|registration|register_save|payments|index|search|page_static|make_money|api_reference|checkfiles|page|contact|contact_send|download1|download_orig|deurl|stream_page|download_versions)$/;
    ...
    if($self->getUser)
    {
        ...
        $t->param( 'admin_panel' => 1, 'xvs_notifications' => $self->{xvs_notifications} ) if $self->{adm} || $filename=~/^admin_/;
        $t->param( 'my_login'           => $self->{user}->{usr_login},
                    ...
                    'tickets_moderator' => $c->{m_e} && $c->{ticket_moderator_ids} && $self->{user}->{usr_id}=~/^$c->{ticket_moderator_ids}$/ ? 1 : 0,
                  );
    }
    return $t;
}
...
sub PrintTemplate
{
	...
	#return $self->PrintTemplatePSGI($template,%par) if $self->{psgi_req};
	...
	foreach my $name (keys %{ $self->{cookies_send} })
	{
		my $c =  $self->{cgi_query}->cookie(-name		=> $name,
											...
											-httponly	=> substr($c->{site_url}, 0, 5) eq "https" ? 1 : 0,
											-secure		=> substr($c->{site_url}, 0, 5) eq "https" ? 1 : 0,
											-samesite	=> 'None',
											);
		push(@Cookies, $c);
	}
	my %hhh;
	#$hhh{'-access_control_allow_origin'} = '*' if $self->f->{op}=~/(upload_result|my_files)/;
	...
	else
	{
		...
		$t->param( 'language' => $self->{language},'language2' => ucfirst $self->{language}, language_lc => lc $self->{language} );
		#my @ll = map{{lang=>$_,lang2=>ucfirst $_}} grep{$_ ne $self->{language}} @{$c->{languages_list}};
		#$t->param(languages => \@ll);

		# my $ll = $self->db->SelectARefCached(60,"SELECT * FROM Languages WHERE lang_active=1 ORDER BY lang_order") if $self->{db} && !$c->{no_lng_sql};
		# my @ll2;
		# for(@$ll)
		# {
...
sub PrintTemplatePSGI
{
  ...
  foreach my $name (keys %{ $self->{cookies_send} })
  {
    $res->cookies->{$name} = {
                             	value   => $self->{cookies_send}->{$name},
                                domain  => ".$self->{domain}",
                                expires => $self->normalizeDate($self->{cookies_exp}->{$name}), # can't use +3d here
                            };
  }
...
sub getIP
{
 ...
 $ip = $ENV{HTTP_X_FORWARDED_FOR} if $ENV{HTTP_X_FORWARDED_FOR} && $ENV{HTTP_SAVE_DATA}; # Google Data Saver
...
sub getCountryCode
{
    ...
    return uc($country) || 'XX';
}

sub logg
{
	my ($self,$name,$msg) = @_;
	my $date = sprintf("%04d-%02d-%02d %02d:%02d:%02d",$self->getTime);
	$name||='misc';
...
{ 
	...
	my $x = scalar @range;
	return join '', map $range[rand $x], 1..($num||1);
}
...
sub api
{
   ...
   unless($self->{ua})
   {
	   ...
	   #$self->{ua}->default_header( 'HDR2D2' => '45' );
   }
...
sub api2
{
	...
	return $self->api($srv->{srv_cgi_url},
						{
							dl_key => $c->{dl_key},
							disk_id => $srv->{disk_id},
							%{$data},
...
sub api_host
{
	...
	return $self->api($host->{host_cgi_url},
						{
							dl_key => $c->{dl_key},
							%{$data},
						}
			...
			else
			{
				$self->db->Exec("INSERT INTO QueueDelete 
								SET file_real_id=?, 
									...
									srv_id=?, 
									del_by=?, 
									del_time=NOW()+INTERVAL ? HOUR", 
								$file->{file_real_id}||$file->{file_id},
								$file->{file_real},
								$file->{srv_id},
								$self->getUserId||0, 
								$self->{delete_disk_time}||$c->{delete_disk_time}||0 );
			}
...
sub DeleteFileDB
{
	...
	$self->db->Exec("INSERT IGNORE INTO FilesTrash 
					SELECT *, 
					NOW() as file_deleted, 
					? as del_by, 
					0 as hide, 
					0 as cleaned 
					FROM Files 
					WHERE file_id=?", $self->getUserId||0, $file->{file_id} );
	...
	# my $srtpath = "$c->{site_path}/srt/$dx/$file->{file_code}_";
	# unlink(<$srtpath*>) if $c->{srt_on};
}
...
{
	my ($self,$file) = @_;
	my $res = $self->api2( $file->{srv_id}, { op => 'del_files', list => "$file->{file_real_id}-$file->{file_real}" });
...
sub PurgeCloudflareCache
{
	...
	#$req->header( 'X-Auth-Email' => $c->{m_i_cf_auth_email} );
	#$req->header( 'X-Auth-Key' => $c->{m_i_cf_auth_key} );
...
sub SendMailQueue
{
	...
	$self->db->Exec("INSERT INTO QueueEmail SET 
					email_to=?,
					email_from=?,
					subject=?,
					body=?,
...
sub SendMailTLS2
{
   #IO::Socket::SSL, Net::SSLeay, Authen::SASL
   require Net::SMTPS;
   my ($self, $mail_to, $mail_from, $subject, $body) = @_;
   my $content_type = $c->{email_html} ? 'text/html' : 'text/plain';
   my $msg = new Net::SMTPS(
                   $c->{smtp_server},
                   Hello => $self->{domain}, 
                   doSSL => 'starttls', 
                   Port => 587
...
{
    my ($self,$file) = @_;
    my $idd = $self->encode_base36( $file->{file_id} );
    $c->{m_j_domain}=~s/http:\/\///;
    my $site_url = $file->{usr_custom_domain} ? "https://$file->{usr_custom_domain}" : $c->{site_url};
    return $c->{m_j_domain} ? "http://$c->{m_j_domain}/$idd" : "$site_url/u/$idd";
}
...
sub makeEmbedCode
{
    ...
    $site_url = $file->{usr_custom_domain} ? "https://$file->{usr_custom_domain}" : $site_url;
    ...
    return qq[<iframe width="$ew" height="$eh" src="$embed_url" scrolling="no" frameborder="0" allowfullscreen="true"></iframe>] if $simple || !$c->{embed_responsive};
    return qq[<div style="position:relative;padding-bottom:56%;padding-top:20px;height:0;"><iframe width="$ew" height="$eh" src="$embed_url" scrolling="no" frameborder="0" allowfullscreen style="position:absolute;top:0;left:0;width:100%;height:100%;"></iframe></div>];
}
...
{ 
 ...
 return '' if $items_per_page eq 'all';
  ...
  my $t='<div class="paging">';
  if($self->f->{fast_paging})
  {
    $t.="<a href='$c->{site_url}/$adm?$par&amp;page=".($current_page-1)."'>&larr; $self->{lang}->{lng_paging_previous}</a>" if $current_page>1;
    $t.="<b>$current_page</b>";
    $t.="<a href='$c->{site_url}/$adm?$par&amp;page=".($current_page+1)."'>$self->{lang}->{lng_paging_next} &rarr;</a>" if $self->f->{fast_paging_next};
    $t.="</div>";
    return $t;
  }
 ...
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=".($current_page-1)."'>&larr; $self->{lang}->{lng_paging_previous}</a>" if $current_page>1;
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=1'>1</a>" if $i1>1;
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=2'>2</a>" if $i1>2;
 $t.="... " if $i1>3;
 ...
 while( $i <= $i2 )
 {
    ...
    {
       $t .= $i==$current_page ?
       "<b>$i</b>" : "<a href='$c->{site_url}/$adm?$par&amp;page=$i'>$i</a>";
    }
    else
    {
 ...
 $t.="... " if $i2<$total_pages-2;
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=".($total_pages-1)."'>".($total_pages-1)."</a>" if $i2<$total_pages-1;
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=$total_pages'>$total_pages</a>" if $i2<$total_pages;
 $t.="<a href='$c->{site_url}/$adm?$par&amp;page=".($current_page+1)."'>$self->{lang}->{lng_paging_next} &rarr;</a>" if $current_page<$total_pages;

 $t.="<small>($total_items $self->{lang}->{lng_paging_total})</small></div>";
...
{
   my ($self, $hash, $id) = @_;
   my ($tt,$md5) = split(/-/,$hash);
   unless($tt && $md5){ $self->{form}->{msg}="Invalid hash";return 0; }
   my ($ip1,$ip) = $self->genIPLogic( $self->getIP );
   my $str = "$id-$ip1-$tt";
   require Digest::MD5;
   ...
   if($tt>time){$self->{form}->{msg}="Skipped countdown";return 0;}
   if($tt<time-60*30){$self->{form}->{msg}="Expired session";return 0;}
   #if($id && $id != $idh){$self->{form}->{msg}="Wrong id";return 0;}
   #if($iph ne $ip1){$self->{form}->{msg}="Wrong IP";return 0;}
...
sub vInfo
{
	...
	$x->{file_size_mb} = $self->makeFileSize( $file->{"file_size$extra"} );
...
sub addTagsToFile
{
    ...
    for my $tag (split(/\s*\,\s*/,$tags))
    {
       ...
       my $tag_id = $self->db->SelectOne("SELECT tag_id FROM Tags WHERE tag_value=?",$tag);
       unless($tag_id)
       {
          $self->db->Exec("INSERT INTO Tags SET tag_value=?",$tag);
          $tag_id = $self->db->getLastInsertId;
       }
       $self->db->Exec("INSERT IGNORE INTO Tags2Files SET file_id=?, tag_id=?",$file_id,$tag_id);
    }
}
...
sub addExtraFileData
{
    my ($self, $f, $file_id) = @_;
      ...
      for my $kk (keys %extra)
      {
          $extra{$kk}=~s/^\s+//g;
          $extra{$kk}=~s/\s+$//g;
          $self->db->Exec("INSERT INTO FilesData SET file_id=?, name=?, value=?", $file_id, $kk, $extra{$kk} ) if $extra{$kk};
      }
}
...
sub getVideoInfo
{
    ...
    #     $file->{srv_htdocs_url} = $self->{servers}->{$file->{srv_id}}->{srv_htdocs_url};
    #     $file->{srv_cgi_url}    = $self->{servers}->{$file->{srv_id}}->{srv_cgi_url};
    #     $file->{disk_id}        = $self->{servers}->{$file->{srv_id}}->{disk_id};
    # }
    
    # $file->{video_img_folder}="$file->{srv_htdocs_url}/i/$file->{disk_id}/$dx";
    
    # $file->{video_img_url}="$file->{video_img_folder}/$file->{file_real}.jpg";
    # $file->{video_thumb_url}="$file->{video_img_folder}/$file->{file_real}_t.jpg";
    # if($file->{file_screenlist})
    # {
    #    $file->{img_screenlist}       = "$file->{video_img_folder}/$file->{file_real}_x.jpg";
    #    $file->{img_screenlist_thumb} = "$file->{video_img_folder}/$file->{file_real}_xt.jpg";
    # }
}
...
sub processAff
{
    ...
    $self->db->Exec("INSERT INTO Stats2
              SET usr_id=?, day=CURDATE(),
                  sales=1, profit_sales=?, refs=$refs
              ON DUPLICATE KEY UPDATE
                  sales=sales+1, profit_sales=profit_sales+?, refs=refs+$refs
             ",$transaction->{aff_id},$money,$money);
...
sub processWebsite
{
    ...
    if($usr_id)
    {
      ...
      $self->db->Exec("INSERT INTO Stats2
                      ...
                      ",$usr_id,$money,$money) if $c->{m_s};

      $self->db->Exec("UPDATE Websites 
                       SET money_sales=money_sales+?, money_profit=money_profit+? 
                       WHERE usr_id=? AND domain=?", $transaction->{amount}, $money, $usr_id, $domain);
    }
}
...
sub genToken
{
    ...
    require Digest::MD5;
    #return Digest::MD5::md5_hex( $self->{user}->{session_id}.$self->{user}->{usr_login}.$self->{user}->{usr_email}.$c->{dl_key}.$self->f->{op} );
    #my @dt = $self->getTime;
    my $x = Digest::MD5::md5_hex( $self->{user}->{session_id}.$c->{dl_key}.$c->{pasword_salt}.join('',$self->getDate) );
    #die $self->{user}->{session_id}.$c->{dl_key}.$c->{pasword_salt}.join('',$self->getDate);# unless $x eq '0be52af7e3882a784d5ee86e8ed9e28e';
    return $x;
}
...
sub checkToken
{
    ...
    {
        #sleep(1);
        #return $self->message("Invalid token");
        return 0;
    }
    return 1;
}
...
sub getPlugins
{
  ...
  #require Data::Dumper;
  #die Data::Dumper->Dump([$ret]);

  return @$ret if wantarray();
  return Session::PluginsList->new($ret); # Scalar context
}
...
sub syncFTPUsers
{
    ...
    my $users = $self->db->SelectARef("SELECT usr_login, usr_ftp_password 
                                      FROM Users 
                                      WHERE usr_status='OK' 
                                      AND (usr_lastlogin > NOW()-INTERVAL 14 DAY OR usr_created>NOW()-INTERVAL 1 DAY)
                                      $filter");
    for(@$users)
    {
    ...
    my $hosts = $self->db->SelectARef("SELECT * FROM Hosts 
                                         WHERE host_ftp=1");
    for my $s (@$hosts)
    {
        my $res = $self->api_host($s->{host_id},
                            {
                               op     => 'add_ftp_users',
                               list  => $list,
                            }
...
sub processVideoList
{
    ...
    for(@$list)
    {
       ...
       #$_->{file_size}     = $self->makeFileSize($_->{file_size});
       ...
       $_->{lng_videoslist_days_ago} = $self->{lang}->{lng_videoslist_days_ago};
...
sub genThumbURLs
{
    ...
    {
      #$file->{video_img_folder}="$c->{m_i_server}/$file->{host_id}/$file->{disk_id}/$dx";
      $file->{iproxy}=1;
    ...
    $file->{video_img_url}="$file->{video_img_folder}/$file->{real}.jpg";
    $file->{video_thumb_url}="$file->{video_img_folder}/$file->{real}_t.jpg";
    push @list, $file->{video_img_url}, $file->{video_thumb_url};
...
sub saveUserData
{
  ...
  else
  {
   ...
   utf8::encode($value);
   $value||='';
   $self->db->Exec("INSERT INTO UserData
                    SET usr_id=?, name=?, value=?
                    ON DUPLICATE KEY UPDATE value=?
                   ", $usr_id, $name, $value, $value);
  }
}
...
sub getCaptionsLinks
{
	...
	# ?c1_file=https://loadsubs.net/movies/files/deadpool-2-en1.srt&c1_label=english
	...
	# my @list = split /\s*\,\s*/, $c->{srt_langs};
	# my $dx = sprintf("%05d",$file->{file_id}/$c->{files_per_folder});
	# #my $srtauto=',"default": true' if $c->{srt_auto_enable};
	# my $dir = "$c->{site_path}/srt/$dx";
	# for(@list)
	# {
	# 	push @arr, { url => "/srt/$dx/$file->{file_code}_$_.vtt", main => "$file->{file_code}_$_.vtt", language=>$_, title => $_ } if -f "$dir/$file->{file_code}_$_.vtt";
	# 	push @arr, { url => "/srt/$dx/$file->{file_code}_$_.srt", main => "$file->{file_code}_$_.srt", language=>$_, title => $_ } if -f "$dir/$file->{file_code}_$_.srt";
	# }
...
sub loadUserData
{
	my ($self,$user) = @_;
	$user||=$self->{user};
	my $data = $self->db->SelectARefCached("SELECT * FROM UserData WHERE usr_id=?",$user->{usr_id});
...
sub checkModSpecialRights
{
    ...
    return 1 if $c->{"$mod\_users"} eq 'special' && ( $user->{data_loaded} ? $user->{$user_field} : $self->db->SelectOneCached("SELECT value FROM UserData WHERE usr_id=? AND name=?",$user->{usr_id},$user_field) );
...
sub selectProxyServer
{
	my ($self, $file) = @_;
	my $sortout = $c->{hls_proxy_random_chance} && int(rand 100)>$c->{hls_proxy_random_chance} ? 'ROUND(20*h.host_out/h.host_net_speed),' : '';
	my $proxylist = $self->db->SelectARef("SELECT p.host_id, 100*host_out/(host_net_speed+1) as net_usage
											FROM Proxy2Files p, Hosts h
											WHERE p.file_id=? 
	...
	$proxy_limit||=1;
	my $use_sticky_proxies='';
	# use sticky proxies if max proxies limit reached and lowest Net Out proxy less than 90%
	if($proxylist && $#$proxylist >= $proxy_limit-1 && $proxylist->[0]->{net_usage} < 90)
	{
		my $pids = join ',', map{$_->{host_id}} @$proxylist;
		$use_sticky_proxies="AND h.host_id IN ($pids)";
	}
	# Choose dedicated proxies for this user, if any
	my $proxy = $self->db->SelectRowCached(30,"SELECT * 
										FROM Hosts h
										WHERE h.host_proxy=1 
										AND host_proxy_users_only LIKE '%,$file->{usr_id},%'
										$use_sticky_proxies
										ORDER BY $sortout RAND()
										LIMIT 1
										");
	
	# Choose regular sticky proxy
	$proxy ||= $self->db->SelectRowCached(20,"SELECT * 
										FROM Hosts h
										WHERE h.host_proxy=1 
										$use_sticky_proxies
										AND host_proxy_users_only='' 
										ORDER BY $sortout RAND()
										LIMIT 1
										");
	
	# Choose regular any proxy
	$proxy ||= $self->db->SelectRowCached(10,"SELECT * 
										FROM Hosts h
										WHERE h.host_proxy=1
										AND host_proxy_users_only='' 
										ORDER BY $sortout RAND()
...
sub genDirectLink
{
	...
	$speed = $self->{transfer_speed} if $self->{transfer_speed}; # Transfers override
	...
	if($c->{proxy_downloads})
	{
		my $proxy = $self->selectProxyServer($file);
		my $srv = $file->{srv_htdocs_url};
		$srv=~s/^https?:\/\///i;
		$srv=~s/^([\w\_\-]+)\..+/$1/i;

		return "$proxy->{host_htdocs_url}/vp/$file->{disk_id}/$dx/$file->{file_real}_$quality/$fname?t=$token&s=$time&e=$expire&f=$file->{file_id}&sp=$speed&i=$ip&srv=$srv" if $proxy && $proxy->{host_htdocs_url};
	}

	return "$file->{srv_htdocs_url}/v/$file->{disk_id}/$dx/$file->{file_real}_$quality/$fname?t=$token&s=$time&e=$expire&v=$file->{view_id}&sp=$speed&i=$ip";
}
...
sub genHLSLink
{
	...
	# MD5 hash version
	#require Digest::MD5;
	#my $token = Digest::MD5::md5_base64("$expire $ip $c->{dl_key}");
	...
	if($do_proxy)
	{
		my $sortout = $c->{hls_proxy_random_chance} && int(rand 100)>$c->{hls_proxy_random_chance} ? 'ROUND(20*h.host_out/h.host_net_speed),' : '';
		my $filter_host='';
		my $proxylist = $self->db->SelectARef("SELECT p.host_id, host_out/(host_net_speed+1) as x
												FROM Proxy2Files p, Hosts h
												WHERE p.file_id=? 
												AND p.host_id = h.host_id
												ORDER BY x",
		...
		my $proxy_limit = $file->{owner_premium} ? $c->{proxy_num_prem} : $c->{proxy_num_reg};
		$proxy_limit||=1;
		# get more proxies if lowest out proxy exceeds 90%+ or limit not reached
		...
		if($proxy)
		{
			$self->db->Exec("INSERT INTO Proxy2Files SET file_id=?, host_id=?
							ON DUPLICATE KEY UPDATE created=NOW()", $file->{file_real_id}, $proxy->{host_id} );
...
sub ViewAdd
{
    ...
    my $referer = $self->getDomain($self->{form}->{referer});
    my $premium = $self->getUser && $self->getUser->{premium};

    $file->{money_code} = 4 if $file->{file_ip} eq $self->getIP;

    $self->db->Exec("INSERT IGNORE INTO Views
                SET file_id=?,
                    ip=INET6_ATON(?),
                    usr_id=?,
                    owner_id=?,
                    ...
                    $self->getIP,
                    $file->{usr_id} || 0,
                    $file->{usr_id} || 0, # Assuming owner_id is same as usr_id, adjust if different
                    $referer || '',
                    $self->{form}->{embed} || 0,
                    $self->{form}->{adb} || 0,
                    $premium || 0,
                    ...
                    $file->{download} || 0,
                    $file->{money_code} || 0,
                    $file->{file_size} || 0 # Assuming file_size is the equivalent of size in the table
                    );

    $file->{view_id} = $self->db->getLastInsertId || 
                       $self->db->SelectOne("SELECT file_id FROM Views WHERE file_id=? AND ip=INET6_ATON(?)", 
                                            $file->{file_id}, $self->getIP);
}

sub getFileRecord
{
	my ($self, $file_code) = @_;
	my $file = $self->db->SelectRowCachedKey("filedl$file_code",120,
		"SELECT f.*, s.*, h.*, u.*, 
			u.usr_login as file_usr_login,
			DATE_FORMAT(file_created,'%b %e, %Y') as file_created_txt,
			UNIX_TIMESTAMP(usr_premium_expire) - UNIX_TIMESTAMP() as exp_sec,
			UNIX_TIMESTAMP() - UNIX_TIMESTAMP(file_last_download) as last_view_sec
		FROM (Files f, Servers s, Hosts h)
		LEFT JOIN Users u ON f.usr_id = u.usr_id
		WHERE f.file_code=?
		AND f.srv_id=s.srv_id
		AND s.host_id=h.host_id",$file_code);
	if($file)
	{
...
sub getPlayVersions
{
	...
	if($file->{"file_size_o"} && $self->webPlayable($file,'o'))
	{
		$play->{o}=1		if $c->{"vid_play_$self->{utype}_o"} || !keys %$play;
		$playprem->{o}=1	if $c->{"vid_play_prem_o"} || !keys %$play;
	}
...
sub webPlayable
{
	...
	return 1 if $file->{"file_size_$q"} && $spec=~/mp4$/i && $spec=~/(h264|av1)/i && $spec=~/aac/i;
	#return 1 if $file->{"file_size_$q"} && $spec=~/(mp4|matroska)/i && $spec=~/(HEVC)/i;# && $spec=~/aac/i;
	return 0;
}
...
sub cloneFile
{
	...
	my $code;
	do {$code = $self->randchar(12)} while ($self->db->SelectOne("SELECT file_id FROM Files WHERE file_code=?",$code));

	$self->db->Exec("INSERT INTO Files 
					SET file_name=?, 
						file_title=?, 
						usr_id=?, 
						srv_id=?,
	...
	if($file->{file_captions})
	{
		unless($file->{host_id} && $file->{disk_id})
		{
			my $srv = $self->db->SelectRowCached("SELECT * FROM Servers WHERE srv_id=?",$file->{srv_id});
			$file->{host_id} = $srv->{host_id};
			$file->{disk_id} = $srv->{disk_id};
		}
		my $data={	file_real_id	=> $file->{file_real_id},
					disk_id			=> $file->{disk_id},
					file_code		=> $file->{file_code},
					file_code_new	=> $code,
					languages		=> $file->{file_captions},
				};
		$self->db->Exec("INSERT INTO QueueTasks SET host_id=?, op=?, file_real_id=?, data=?, priority=2", 
					$file->{host_id}, 'vtt_clone', $file->{file_real_id}, encode_json($data) );
	}
...
sub logFile
{
	my ($self, $file_real_id, $usr_id, $event) = @_;
	return unless $c->{file_logs_on};
	return if $c->{file_logs_usr_ids} && $usr_id && $usr_id !~ /^($c->{file_logs_usr_ids})$/;
	$self->db->Exec("INSERT INTO FileLogs SET file_real_id=?, event=?", $file_real_id, $event ) if $file_real_id && $event;
}

sub checkEventRate
{
	my ($self, $usr_id, $name, $limit) = @_;
	my $num = $self->db->SelectOne("SELECT value 
									FROM StatsMiscMin 
									WHERE usr_id=? 
									AND minute=DAYOFYEAR(NOW())*24*60 + HOUR(NOW())*60 + MINUTE(NOW()) 
									AND name=?", $usr_id, $name)||0;
	return $num if $num >= $limit;
	$self->db->Exec("INSERT INTO StatsMiscMin 
					SET usr_id=?, 
					minute=DAYOFYEAR(NOW())*24*60 + HOUR(NOW())*60 + MINUTE(NOW()), 
					name=?, 
					value=1 
					ON DUPLICATE KEY UPDATE value=value+1", $usr_id, $name);
	return $num;
}

sub addTaskTimeslider
{
	my ($self, $file) = @_;
	if($file->{file_code} eq $file->{file_real})
	{
		my $data = {file_real_id=> $file->{file_real_id},
					...
					m_z_cols	=> $c->{m_z_cols},
					m_z_rows	=> $c->{m_z_rows},
					original_priority => $file->{original_priority}||0 };
		$self->db->Exec("INSERT INTO QueueTasks SET host_id=?, op=?, file_real_id=?, data=?, priority=1", 
					$file->{host_id}, 'gen_slides', $file->{file_real_id}, encode_json($data) );
	}

	$self->db->Exec("INSERT INTO QueueVTT 
						SET file_real_id=?,
						file_code=?,
						host_id=?,
						disk_id=?,
						...
						$file->{host_id},
						$file->{disk_id},
						$self->getTimesliderVTT($file),
					);
}

sub addTaskScreenlist
{
	my ($self, $file) = @_;
	my $data_screen;
	for (qw(m_x_width
	...
	{
		$data_screen->{$_}=$c->{$_} if $c->{$_};
	}
	my $info='';
	utf8::decode($file->{file_title});
	utf8::decode($file->{file_name});
	$info.="File Title: $file->{file_title}\n" if $c->{m_x_file_title} eq 'title';
	$info.="File Name: $file->{file_name}\n" if $c->{m_x_file_title} eq 'name';
	my $fsize = $self->makeFileSize($file->{file_size_o});
	$info.="File Size: $fsize\n" if $c->{m_x_file_size} eq '1';
	$info.="File Size: $fsize ($file->{file_size_o} bytes)\n" if $c->{m_x_file_size} eq '2';
	$file->{file_length1} = sprintf("%.0f", $file->{file_length} );
	$file->{file_length2} = sprintf("%02d:%02d:%02d",int($file->{file_length}/3600),int(($file->{file_length}%3600)/60),$file->{file_length}%60);
	$info.="Video Length: $file->{file_length2}\n" if $c->{m_x_file_length} eq '1';
	$info.="Video Length: $file->{file_length2} ($file->{file_length1} seconds)\n" if $c->{m_x_file_length} eq '2';
	my $vi = $self->vInfo($file,'o');
	$info.="Resolution: $vi->{vid_width}x$vi->{vid_height} px\n" if $c->{m_x_file_resolution} eq '1';
	$info.="Resolution: $vi->{vid_height}p\n" if $c->{m_x_file_resolution} eq '2';
	utf8::decode($file->{file_descr});
	my $descr=$file->{file_descr};
	$descr = substr($file->{file_descr}, 0, $c->{m_x_descr_limit}) if $c->{m_x_descr_limit} && $file->{file_descr};
	$info.="$descr\n" if $c->{m_x_descr_on};
	my $lines = () = $info =~ /\n/gi;
	#print STDERR "l1:$lines\n$info\n";
	$data_screen->{info_height} = ($lines+1)*($c->{m_x_text_size}+4);
	$data_screen->{info_height} = 60 if $data_screen->{info_height}<60;
	#print STDERR "l2:$data_screen->{info_height}\n";
	my $data = {file_real_id=> $file->{file_real_id},
				file_real	=> $file->{file_real},
				disk_id		=> $file->{disk_id},
				file_length => $file->{file_length},
				...
				# file_length => $file->{file_length},
				# file_size	=> $file->{file_size_o},
				# file_name	=> $file->{file_name},
				# file_title	=> $file->{file_title},
				# file_descr	=> $file->{file_descr},
				};
	$self->db->Exec("INSERT INTO QueueTasks SET host_id=?, op=?, file_real_id=?, data=?, priority=2", 
				$file->{host_id}, 'gen_screenlist', $file->{file_real_id}, encode_json($data) );
}

sub getTimesliderVTT
{
	my ($self, $file) = @_;
	sub formatTime
	{
	...
	my $url = "$file->{file_real}_z.jpg";
	$url = "$c->{m_i_server}/$file->{file_code}_z.jpg" if $c->{m_i} && $c->{m_i_server};
	for my $i (0..$frames-1)
	{
...
sub deleteVTT
{
	...
	unless($file->{disk_id} && $file->{host_id})
	{
		my $srv = $self->db->SelectRowCached("SELECT * FROM Servers WHERE srv_id=?",$file->{srv_id});
		($file->{disk_id}, $file->{host_id}) = ($srv->{disk_id}, $srv->{host_id});
	}
	my $data = {file_real_id=> $file->{file_real_id},
				file_code	=> $file->{file_code},
				disk_id		=> $file->{disk_id},
	...
	$self->db->Exec("INSERT INTO QueueTasks SET host_id=?, op=?, file_real_id=?, data=?, priority=2", 
				$file->{host_id}, 'vtt_delete', $file->{file_real_id}, encode_json($data) );

	$file->{file_captions} = join '|', grep{$_ ne $language} split(/\|/, $file->{file_captions});
	$self->db->Exec("UPDATE Files SET file_captions=? WHERE file_id=?", $file->{file_captions}, $file->{file_id}) unless $language eq 'all';
}
...
sub AUTOLOAD {
  # Forward method calls to appropriate plugins
