...
if($ENV{HTTP_REFERER} && $sub && !$ses->{ref_ok} && $op!~/(admin_torrents)/)
{
   ...
   #$pass = 1 if !$pass && $db->SelectOne("SELECT srv_id FROM Servers WHERE srv_cgi_url LIKE 'http://$dm%'");
...
# sub AdminDownloads
# {
#    my $filter_user = "AND d.usr_id=$f->{usr_id}" if $f->{usr_id}=~/^\d+$/;
#    my $filter_owner = "AND d.owner_id=$f->{owner_id}" if $f->{owner_id}=~/^\d+$/;
#    my $filter_ip = "AND d.ip=INET_ATON('$f->{ip}')" if $f->{ip}=~/^[\d\.]+$/;
#    my $filter_file = "AND d.file_id=$f->{file_id}" if $f->{file_id}=~/^\d+$/;
#    my $filter_referer = "AND d.referer LIKE '%$f->{referer}%'" if $f->{referer}=~/^[\w\s\.\-]+$/;

#    my $list = $db->SelectARef("SELECT *, INET_NTOA(ip) as ip 
#                                FROM Downloads d, Files f
...
#     for(@$list)
#     {
#         $_->{download_url} = $ses->makeFileLink($_);
#         $_->{size} = $ses->makeFileSize($_->{size});
#     }
...
sub AdminTransferList
{
   ...
   # my $srv_list_to = $db->SelectARef("SELECT s.srv_id, s.srv_name,
   ...
   #                                 GROUP BY srv_id2
   #                                ");
   # my $srv_list_from = $db->SelectARef("SELECT s.srv_id, s.srv_name,
   #                                 SUM(IF(q.status='PENDING',1,0)) as num_pending,
   #                                 SUM(IF((q.status='MOVING' AND q.updated>=NOW()-INTERVAL 60 SECOND),1,0)) as num_moving,
   #                                 SUM(IF((q.status='MOVING' AND q.updated<NOW()-INTERVAL 60 SECOND),1,0)) as num_stucked,
   #                                 SUM(IF(q.status='ERROR',1,0)) as num_error
...
sub AdminFiles
{
	...
	if($f->{dmca_rename_selected} && $f->{file_id})
	{
		...
		for my $file (@$files)
		{
			...
			if($c->{m_a_rename_on_fs})
			{
				...
				$db->Exec("UPDATE Files SET file_code=?, file_real=? WHERE file_id=?", $code, $code, $file->{file_id} );
				$db->Exec("UPDATE Files SET file_real=? WHERE file_real=?", $code, $file->{file_real});
				$db->Exec("INSERT INTO FilesDMCARenamed SET usr_id=?, file_id=?, file_code_old=?, file_code_new=?", $file->{usr_id}, $file->{file_id}, $file->{file_code}, $code );
			}
			else
			{
				$db->Exec("UPDATE Files SET file_code=?, file_status='OK' WHERE file_id=?", $code, $file->{file_id});
				$db->Exec("INSERT INTO FilesDMCARenamed SET usr_id=?, file_id=?, file_code_old=?, file_code_new=?", $file->{usr_id}, $file->{file_id}, $file->{file_code}, $code );
			}

			push @list, {old_code => $file->{file_code}, new_code => $code};
		}
		#$ses->redirect_msg("?op=admin_files",($#$files+1)." files were DMCA renamed");
   ...
   if($f->{del_special}=~/^\w$/i && $f->{file_id})
   {
      ...
      my $ids = join(',',grep{/^\d+$/}@{&ARef($f->{file_id})});
      $ses->redirect($c->{site_url}) unless $ids;
      my $files = $db->SelectARef("SELECT * FROM Files f, Servers s WHERE file_id IN ($ids) AND f.srv_id=s.srv_id");
      for my $file (@$files)
      {
   ...
   if($f->{rethumb_selected})
   {
	...
	if($f->{reslide_selected})
	{
		...
		for my $file (@$files)
		{
			$ses->addTaskTimeslider($file);
		}
		$ses->redirect_msg("?op=admin_files",@$files." files slides will be regenerated");
	}
...
#                                       ROUND(SUM(dt.bandwidth)/1024/1024) as bandwidth
#                                LEFT JOIN DailyTraffic dt ON (f.file_id=dt.file_id AND dt.dayhour > TO_DAYS(CURDATE())*24-24*7 )
   $f->{ip}=~s/[^\w\.\:]+//g;
   my $filter_ip     = "AND f.file_ip=INET6_ATON('$f->{ip}')" if $f->{ip};
   ...
   for(@$files)
   {
      ...
      $_->{download_link} = $ses->makeFileLink($_);
      $_->{file_downloads}=$_->{file_downloads}||$_->{file_downloads_full} ? "$_->{file_downloads_full} / $_->{file_downloads}" : "";
...
sub AdminUserEdit
{
    ...
    push @smods,'usr_streams'   if $c->{m_q} && $c->{m_q_users} eq  'special';
    ...
    # if($f->{reset_downloads})
    # {
    #    $db->Exec("UPDATE Downloads SET size=0 WHERE usr_id=?",$f->{usr_id});
    #    $ses->redirect("?op=admin_user_edit&usr_id=$f->{usr_id}");
    # }
...
sub AdminServers
{
	...
	if($f->{check_file_to_db})
	{
		...
		for my $srv_id (@{ARef($f->{srv_id})})
		{
			...
			#$res=~/^OK:(.*)$/ ? print" OK. Found & fixed bad files2: $1<br>" : 
			print"$res<br>";
		}
	...
	for my $s (@$servers)
	{
		...
		#$s->{spec}=" <sup style='color:#363'>buffer</sup>" if $s->{srv_buffer};
		#$s->{spec}=" <sup style='color:#633'>encoder</sup>" if $s->{srv_encoder};
	...
	# If there're no encoding servers then show warning
...
sub AdminServerAdd
{
   ...
   if(!$f->{srv_id})
   {
      ...
      #my $disk_id = $db->SelectOne("SELECT MAX(disk_id) FROM Servers WHERE host_id=?",$f->{host_id});
...
sub AdminServerImport
{
   ...
   $ses->message("Error when requesting API.<br>$res") unless $res=~/^\[/; #$res=~/^OK/;
   #my ($data) = $res=~/^OK:(.*)$/;
   require JSON;
...
sub AdminSettings
{
    if($f->{ipblock_update} && $c->{m_7})
    {
    	...
    	for my $dbn ('ipproxy','ipserver','iptor')
    	{
    		...
    		my $res = $ua->get("https://sibsoft.net/cgi-bin/client.cgi?op=download_ipblock_db&id=$ses->{cliid}&dbname=$dbn")->content; #, ':content_file' => $filename."new"
   ...
   if($f->{save})
   {
	  ...
	  # Try to write to logs first to check disk is not full
      ...
      if($f->{update_fs_config})
      {
          ...
          for(@$servers)
          {
             ...
             # Restart enc.pl
             # $db->Exec("INSERT IGNORE INTO QueueEncoding SET file_real_id=0, quality='n', file_real='RESTART', file_id=0, srv_id=?, premium=1, created=NOW()-INTERVAL 6 MONTH",$_->{srv_id});
         }
   ...
   #$c->{"epr$c->{max_enc_processes}"}=' selected';
   ...
   for my $q (@{$c->{quality_letters}})
   {
   		...
   		#$x->{"vid_quality_mode_$x->{vid_quality_mode}"}=' checked';
   		for(qw( vid_enc_anon
   				vid_enc_reg
   				vid_enc_prem
   				vid_play_anon
...
sub AdminStats
{
   ...
   for my $x (@list2)
   {
      ...
      # $x->{$_}||='' for qw(uploads deleted downloads views registered);
      # for (qw(paid payout profit profit_total))
      # {
      #   $x->{$_}='' if $x->{$_}==0;
      # }
   }
...
#   for my $i (0..$#tree)
#   {
#       $tree[$i]->{up}=$tree[$i-1]->{cat_id} if $i && $tree[$i-1]->{cat_parent_id}==$tree[$i]->{cat_parent_id};
#       $tree[$i]->{down}=$tree[$i+1]->{cat_id} if $i<$#tree && $tree[$i+1]->{cat_parent_id}==$tree[$i]->{cat_parent_id};
#   }
...
sub AdminSQLStats
{
    ...
    # if($f->{exec} eq 'opt' && $f->{table}=~/^\w+$/i)
    # {
    #     $db->Exec("OPTIMIZE TABLE $f->{table}");
    #     $ses->redirect('?op=admin_sql_stats');
    # }
...
sub AdminMisc
{
    ...
    if($f->{iproxy_config})
    {
    	#~^/1/	http://s1.xvs.tt; host_htdocs_url
...
sub AdminDomains
{
	if($f->{set_active} || $f->{set_off})
	{
		my $ids = join(',',grep{/^\d+$/}@{&ARef($f->{dom_id})});
		my $status='OFF';
		$status='ACTIVE' if $f->{set_active};
		$db->Exec("UPDATE Domains SET dom_status='$status', dom_error='' WHERE dom_id IN ($ids)");
		return $ses->redirect("?op=admin_domains&token=".$ses->genToken);
	}
	if($f->{delete})
	{
		my $ids = join(',',grep{/^\d+$/}@{&ARef($f->{dom_id})});
		my $list = $db->SelectARef("SELECT * FROM Domains WHERE dom_id IN ($ids)");
		for my $d (@$list)
		{
			if($d->{dom_cf_id})
			{
	...
	$f->{per_page}||=100;
	my $filter_user;
	$filter_user="AND d.usr_id=$f->{usr_id}" if $f->{usr_id}=~/^\d+$/;
	my $list = $db->SelectARef("SELECT d.*, u.usr_login
								FROM Domains d
	...
	my $total = $db->SelectOneCached("SELECT COUNT(*)
										...
										WHERE 1
										$filter_user");
	$ses->PrintTemplate("admin_domains.html",
						list	=> $list,
						paging	=> $ses->makePagingLinks($f,$total),
						);
}
...
sub getXVSNews
{
	my $nfile = "$c->{cgi_path}/logs/xvideosharing_notifications.json";
	my $update;
	$update=1 unless -f $nfile;
	my $ftime = (lstat($nfile))[9]; # last modify time
	$update=1 if (time - $ftime) > 60*60; # reload once an hour
	if($update)
	{
		require LWP::UserAgent;
		my $ua = LWP::UserAgent->new(timeout => 10, agent=>$c->{lwp_user_agent});
		$ua->get('https://sibsoft.net/xvideosharing_notifications.json', ':content_file' => $nfile);
	}
	return unless -f $nfile;
	
	open FILE, $nfile;
	my $data = join '', <FILE>;
	close FILE;
	require JSON::XS;
	my $dd = eval { JSON::XS::decode_json($data) };
	return if $@;
	my $last_not_id = $ses->getCookie('adm_not_id') || 0;
	for(@$dd)
	{
		push @{$ses->{xvs_notifications}}, $_ if $_->{id} > $last_not_id;
	}
	$ses->{xvs_notifications}->[-1]->{btn}=1 if $ses->{xvs_notifications};
}