...
sub ProcessFile
{
   ...
   if($notvideo && !$c->{allow_non_video_uploads})
   {
	...
	{
		my $chunk=4096; # read only first and last X bytes from file
	...
	#$file->{file_captions} = join '|', keys %{$file->{srt_hash}};
	my @arr;
...
sub SaveFile
{
   ...
   # # Save captions
   # for my $lng (keys %{$file->{srt_hash}})
   # {
   # 	my $vtt = "$dirup/$file->{file_code}_$lng";
   # 	open FILE, ">$vtt";
   # 	print FILE $file->{srt_hash}->{$lng};
   # 	close FILE;
   # }
...
sub makeSnap
{
    ...
    $fast=1; # always use Fast since ffmpeg 2.1
...
sub createTimeslides
{
	...
	return 0 if $file->{file_path} && -f $slides_file; # skip if encode process and slide already exist
...
sub getVideoInfo
{
	...
	require JSON;
 	...
 	my @subs   = grep{$_->{codec_type} eq "subtitle"} @{$x->{streams}}; # && $_->{codec_name}!~/^(hdmv_pgs_subtitle|dvd_subtitle)$/i
 	...
 	if(@audios >= 2)
 	{
 	  my $ii=0; my @il=('lat','swe','nor','mlt','ndo','lug','kua','ina','aka'); # list of language replacements when language name missing in source
 	  my $i=0;
 	  for(@audios)
 	  {
 	  	$_->{ind} = $i++;
 	  ...
 	  #$f->{maps} = '-map 0:0 '.join ' ', map{"-map 0:$_->{index} -metadata:s:a:$_->{ind} language='$_->{lang}'"} @audios;
 	  $f->{audios} = \@audios;
 	}
...
sub getVideoSRTs
{
	...
	for( grep{$_->{codec_type} eq 'subtitle'} @{$f->{file_spec_json}->{streams}} )
	{
		...
		#$slang = shift @extra_langs if $hh->{$slang};
