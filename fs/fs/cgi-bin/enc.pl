...
while(++$cycles < 1000)
{
 ...
 if($error)
 {
     ...
     unlink("$dir_uploads/$file_real\_$type.mp4");
     unlink($filenew);
...
sub EncodePreview
{
 ...
 my $srcs=join '|', @arr;
 `$c->{ffmpeg} -fflags +discardcorrupt -i "concat:$srcs" -c copy -bsf:a aac_adtstoasc -movflags faststart -f mp4 -y $dir_uploads/$code\_p`;
