...
sub TorrentUpload
{
    ...
    $dd->{downloadLimited} = \1 if $dd->{downloadLimit};
    $dd->{uploadLimited}   = \1 if $dd->{uploadLimit};
    $dd->{downloadLimit} = int $dd->{downloadLimit} if $dd->{downloadLimit};
    $dd->{uploadLimit}   = int $dd->{uploadLimit} if $dd->{uploadLimit};
    $dd->{'peer-limit'}  = int $dd->{'peer-limit'} if $dd->{'peer-limit'};
    $id = int $id;
